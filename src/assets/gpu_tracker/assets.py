...
async def gpu_tracker__easypc_raw(context: dg.AssetExecutionContext):
    async with open(
        settings.BASE_DIR / "src/assets/gpu_tracker/datablitz_products.graphql"
    ) as f:
        query = await f.read()
    ...
    async with AsyncSession(
        base_url="https://easy-pcs.myshopify.com",
        allow_redirects=True,
        impersonate="chrome",
    ) as session:
        while True:
...
@dg.asset(
    ...
    deps=[
        "gpu_tracker__dynaquest_raw",
        "gpu_tracker__datablitz_raw",
        "gpu_tracker__easypc_raw",
    ],
)
